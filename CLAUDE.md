...
No automated tests exist in the MVP outside `vorcall-voice`, `vorcall-core`, `vorcall-release`, `vorcall-app`, `vorcall-hotkey` and `vorcall-screen`'s unit tests. `cargo xwin clippy --target x86_64-pc-windows-msvc -p vorcall-screen` (from `client/`) is the only check available for the Windows capture backend on this machine; the macOS backend compiles only on `release.yml`'s macOS runner (a dry-run dispatch, no tag needed).
...
- After a proto change, regenerate the smoke suite's pb2: `uvx --from grpcio-tools python -m grpc_tools.protoc -I proto --python_out=$HOME/.cache/vorcall-smoke proto/vorcall.proto`; the smoke suite now also drives the screen-share signalling frames.
