...
Vorcall: a private chat for a friend group, invite-only accounts, rooms (a mandatory `general` plus public rooms anyone can create or join) and two-person DMs, each with a presence sidebar and a voice channel (push-to-talk or voice activation, over a UDP media relay). Native Rust desktop client (iced), ASP.NET Core (.NET 10) backend, protobuf frames over a single WebSocket. MVP scope only — see `README.md` for what exists and what deliberately doesn't.
