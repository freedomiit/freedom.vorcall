...
///
/// `show()` opens a D-Bus connection and waits for the reply on Linux, so it
/// runs on a blocking thread instead of on the executor. On macOS the
/// notification is delivered when the returned handle drops, which is what
/// happens here; the handle type differs per platform, hence the discard.
