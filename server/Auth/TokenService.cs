    ...
    ILogger<TokenService> logger)
{
    ...
    public async Task<RefreshOutcome> RefreshAsync(string plaintext, DateTime now)
    {
        ...
        // The DisabledAt check is defence in depth, not the gate: AccountService.RefreshAsync
        // checks a disabled account first and answers 403 before ever calling here, so this arm
        // is unreachable through the HTTP endpoint today. A locked account's tokens are revoked
        // by "users disable" anyway, which is why an unknown-token answer costs it nothing.
