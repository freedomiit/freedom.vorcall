...
// The relay's totals, summed across every room. Drops are keyed by reason rather than carried as
// fields so the exposition can walk them: the reason set is fixed and small, unlike the rooms,
// which never become labels.
...
public sealed class VoiceRelay(VoiceOptions options, ILogger<VoiceRelay> logger) : BackgroundService
{
    ...
    // The same figures summed across rooms, kept alongside rather than added up per scrape:
    // _counters never drops a room, so summing it would cost more the longer the process runs.
    private readonly RoomCounters _totals = new();
    ...
    public RelaySnapshot Snapshot()
    {
        ...
        var drops = new Dictionary<string, long>(StringComparer.Ordinal)
        {
            ...
            ["room_gone"] = Interlocked.Read(ref _totals.DropRoomGone),
    ...
    // Every per-room figure moves the relay-wide one with it, so the two can never drift and the
    // scrape never has to walk the rooms.
    private static void Bump(ref long room, ref long total)
    {
        Interlocked.Increment(ref room);
        Interlocked.Increment(ref total);
    }

    private static void BumpBy(ref long room, ref long total, long amount)
    {
        Interlocked.Add(ref room, amount);
    ...
    private async Task ForwardAsync(Socket socket, Inbound inbound, byte[] buffer, byte[] scratch, byte[] plaintext, CancellationToken stoppingToken)
    {
        ...
        {
            Bump(ref counters.DropRoomGone, ref _totals.DropRoomGone);
            return;
        }
    ...
    private void LogMetrics()
    {
        ...
        foreach (var (roomId, members) in _rooms)
        {
            ...
            logger.LogInformation(
                ...
                Interlocked.Read(ref counters.DropRoomGone),
