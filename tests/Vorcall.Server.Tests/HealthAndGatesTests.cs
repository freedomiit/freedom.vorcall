...
public sealed class HealthAndGatesTests(ServerFixture fixture)
{
    ...
    // The positive control of the two refusals above, and the pilot of the channels harness: both
    // gates passed, the upgrade is accepted and the Hello sequence answers with the whole server.
    [Fact]
    public async Task WebSocket_upgrade_with_both_gates_answers_the_hello_sequence()
    {
        var alice = await Accounts.RegisterAsync(Server, "alice");
        await using var a1 = await WsClient.ConnectAsync(Server, alice);

        var session = a1.Session;
        Assert.Equal(ChannelKind.Text, session.General.Kind);
        Assert.True(session.Member(alice.UserId).Online, "the reader is not online in its own snapshot");
        Assert.NotEqual(0L, session.Everyone.Id);

        // A fresh account's read cursor starts at the newest message, so it owes nothing.
        var read = session.Read(session.GeneralId);
