...
public sealed class ProtocolTests(ServerFixture fixture)
{
    ...
    public async Task A_frame_before_hello_answers_fatal_PROTOCOL_and_closes_1008()
    {
        ...
        // The channel id never reaches a handler: the first frame has to be Hello.
        await ws.SendAsync(Frames.Send("too early", channelId: 1));
        await ws.ExpectErrorAsync(ErrorCode.Protocol, fatal: true);
    ...
    public async Task A_second_session_replaces_the_first_and_nobody_else_notices()
    {
        ...
        // The replacement re-reads the whole server and the account never stopped being online, so
        // both members are listed with a session.
        var session = await a2.ReadHelloAsync(alice);
        Assert.True(session.Member(alice.UserId).Online);
        Assert.True(session.Member(carol.UserId).Online);
        await c1.QuietAsync();

        var text = $"back again {Frames.NowMs()}";
        await a2.SendAsync(Frames.Send(text, session.GeneralId));
        ...
        Assert.Equal(session.GeneralId, mirror.ChannelId);
    }

    [Fact]
    public async Task A_disconnect_reaches_the_remaining_members_as_MemberUpdated_offline()
    {
        await using var party = await Party.ConnectAsync(Server, "alice", "carol");
        await party.Client(1).CloseAsync();
        await party.Client(0).ExpectMemberUpdatedAsync(party.Account(1), online: false);
    }
}
