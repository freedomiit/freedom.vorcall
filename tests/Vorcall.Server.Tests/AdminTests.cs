...
// The admin endpoints are the CLI's only reach into the live server; the CLI's database half of
// a ban, an unban and an invite revocation is replayed here through the host's own services.
[Collection(ServerCollection.Name)]
public sealed class AdminTests(ServerFixture fixture)
{
    private const int Kicked = 4001;
    private const int Banned = 4003;
    ...
    [Fact]
    public async Task Ban_closes_with_4003_and_refuses_login_refresh_and_upgrade_until_unbanned()
    {
        var carol = await Accounts.RegisterAsync(Server, "banc");
        await using var ws = await WsClient.ConnectAsync(Server, carol);
        var access = carol.Access;
        var refresh = carol.Refresh;

        // What "users ban" writes before it talks to the server.
        ...
        var kick = await KickAsync(Server, carol.UserId, Banned, reason: null);
        Assert.Equal(HttpStatusCode.OK, kick.Status);
        Assert.True(Closed(kick));
        await ws.ExpectClosedAsync(Banned);
        ...
        Assert.Equal("refresh token is invalid", refreshed.Detail);

        var upgrade = await Assert.ThrowsAsync<InvalidOperationException>(() => WsClient.OpenAsync(Server, access, "banc-banned"));
        ...
        // What "users unban" does.
        await SetDisabledAtAsync(Server, carol.UserId, null);
        Assert.Equal(HttpStatusCode.NoContent, (await RefreshAccountAsync(Server, carol.UserId)).Status);

        await Accounts.LoginAsync(Server, carol);
        await using var back = await WsClient.ConnectAsync(Server, carol, "banc-back");
    }

    // The CLI's live kick can fail (no admin key, wrong AdminUrl); the ban must still reach an
    // open socket on its own.
    [Fact]
    public async Task Ban_without_the_kick_closes_the_socket_at_its_next_write_frame()
    {
        var dave = await Accounts.RegisterAsync(Server, "band");
        ...
        await ws.SendAsync(Frames.Send("still here"));
        await ws.ExpectClosedAsync(Banned);
    ...
    public void IsCliInvocation_recognises_only_the_two_admin_verbs(bool expected, params string[] args)
    {
