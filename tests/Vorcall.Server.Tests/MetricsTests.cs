...
public sealed class MetricsTests(ServerFixture fixture)
{
    ...
    public async Task Metrics_exposes_the_prometheus_text_format_and_counts_a_connected_client()
    {
        ...
        // The gauge counts every channel the mirror holds, DMs included, so the seeded text and
        // voice channels are already there; rooms are gone and so is the gauge that counted them.
        Assert.True(Sample(during, "vorcall_channels") >= 2, $"vorcall_channels is {Sample(during, "vorcall_channels")}");
        Assert.DoesNotContain("vorcall_rooms", during);

        // The relay's drop reasons, every one of them emitted whether it has fired or not.
        Assert.Contains("vorcall_relay_drops_total{reason=\"channel_gone\"} ", during);
        Assert.Contains("vorcall_relay_drops_total{reason=\"muted\"} ", during);
        Assert.DoesNotContain("reason=\"room_gone\"", during);
    }
